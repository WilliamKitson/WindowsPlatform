...
FacadeWindowKeysTest::FacadeWindowKeysTest(HINSTANCE hInstanceValue, int nCmdShowValue)
	: hInstance{ hInstanceValue }, nCmdShow{ nCmdShowValue }, successes{ 0 }
{
}

FacadeWindowKeysTest::~FacadeWindowKeysTest()
{
}

std::string FacadeWindowKeysTest::test()
{
	WindowsPlatform::SubordinateFacade* unit = new WindowsPlatform::SubordinateImplimentation(
		hInstance,
		nCmdShow,
		"facade window keys test"
	);
	...
	return "facade windows keys test failed\n";
}

void FacadeWindowKeysTest::postDown(HWND window, WindowsPlatform::WindowsButtons button)
{
	if (isSysKey(button))
	{
		PostMessage(
...
void FacadeWindowKeysTest::postUp(HWND window, WindowsPlatform::WindowsButtons button)
{
	if (isSysKey(button))
	{
		PostMessage(
...
bool FacadeWindowKeysTest::isSysKey(WindowsPlatform::WindowsButtons button)
{
...
int FacadeWindowKeysTest::getKeyCode(WindowsPlatform::WindowsButtons button)
{
	switch (button)
	{
