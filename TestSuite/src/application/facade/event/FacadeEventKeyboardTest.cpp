...
std::string FacadeEventKeyboardTest::test()
{
	...
	{
		postDown(unit->getWindow(), (windowsPlatform::Keys)i);
		unit->update();
		successes += unit->getKeyboard((windowsPlatform::Keys)i) == true;

		postUp(unit->getWindow(), (windowsPlatform::Keys)i);
		unit->update();
		successes += unit->getKeyboard((windowsPlatform::Keys)i) == false;
	}

	delete unit;
	unit = nullptr;

	if ((successes / 2) == (int)windowsPlatform::Keys::size)
	{
		return std::string();
	}
...
void FacadeEventKeyboardTest::postDown(HWND window, windowsPlatform::Keys key)
{
	if (isSysKey(key))
	{
		PostMessage(
...
void FacadeEventKeyboardTest::postUp(HWND window, windowsPlatform::Keys button)
{
	if (isSysKey(button))
	{
		PostMessage(
...
bool FacadeEventKeyboardTest::isSysKey(windowsPlatform::Keys button)
{
	if (button == windowsPlatform::Keys::keyboard_F10)
	{
		return true;
	}

	if (button == windowsPlatform::Keys::keyboard_alt)
	{
		return true;
	}
...
WPARAM FacadeEventKeyboardTest::getKeyCode(windowsPlatform::Keys key)
{
	switch (key)
	{
