...
KeyboardReleaseTest::KeyboardReleaseTest()
	: successes{ 0 }
{
}
...
std::string KeyboardReleaseTest::test()
{
	...
	for (int i{ 0 }; i < (int)windowsPlatform::KeyboardKeys::size; i++)
	{
		...
		unit.release(getWparam((windowsPlatform::KeyboardKeys)i));
	...
	return "keyboard release test failed\n";
}

WPARAM KeyboardReleaseTest::getWparam(windowsPlatform::KeyboardKeys key)
{
	switch (key)
	{
